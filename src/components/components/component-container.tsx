...
export default function ComponentContainer({ components, updateCallback }: ComponentContainerProps) {
    return (
        <div className="grid grid-cols-3 justify-between m-2 p-2">
            {components.map((component, index) => (
                <ComponentCard key={index} component={component} updateCallback={updateCallback} />
            ))}
        </div>
    )
}